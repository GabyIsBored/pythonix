...
pFont = ('Yu Gothic Ui', 16)

def changePage(currentFrame, nextFrame):
    nextFrame.pack(expand=True, fill='both')
    currentFrame.pack_forget()


# Frames
mainFrame = tb.Frame(root)              # 1. Browse sub-chapter
v_frame = tk.Frame(root, bg='#D9D9D9')  # 1. Variables
a_frame = tk.Frame(root, bg='#D9D9D9')  # 2. Affectation d'un variable

# Widgets (Browse sub-chapter)
button = tb.Button(mainFrame, text='Unit 1 : variable', command=lambda: changePage(mainFrame, v_frame))
button.pack(pady=25)
...
"""
p_text2 = """
Dans ce block de code, le tiroir nom a pour valeur “John” et le tiroir age a pour valeur 32.
...
"""
q_text = """
Lorsque on execute ce programme, on reçois une erreur.
...
# Set background color for v_frame and a_frame
v_frame.configure(bg='#D9D9D9')
a_frame.configure(bg='#D9D9D9')
