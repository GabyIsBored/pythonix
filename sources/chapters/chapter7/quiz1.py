...
class Content(tk.Frame):
    def __init__(self, master: tk.Tk):
        ...
        self.unit_content='''**Declarez et executez la fonction 'setup' **

 setup:
    print('App has been setup')
...
'''
        ...
        self.widget.window_create(3.0, window=self.entry1)
        self.entry2 = tk.Entry(self.mainFrame)
        self.widget.window_create(3.6, window=self.entry2)
        self.entry3 = tk.Entry(self.mainFrame)
        self.widget.window_create(5.0, window=self.entry3)
    ...
    def check(self):
        if "def" in self.entry1.get() and "()" in self.entry2.get() and "setup()" in self.entry3.get():
